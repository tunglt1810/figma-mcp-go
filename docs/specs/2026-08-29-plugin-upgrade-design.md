...
|---|---|---|
| Tools | 63 | 78 |
| `internal/tools/testdata/tools_schema.json` sha256 | — | `909a1e5a4e8d74c59825658338f45a7eb4906a6610fb341e7a7cb7608798bb59` |
| Plugin tests | 352 across 13 files | 680 across 33 files |
| Plugin source modules (non-test) | 21 | 38 |

The "After" column covers the follow-up phase as well — the items this document's
plan first listed as Remaining. At the end of the original four phases the numbers
were 76 tools, 599 tests across 27 files, and 33 modules.
...
Pairing is still rejected, but the risk is no longer silent. The default bind is
loopback, where it costs nothing; what is dangerous is `--ip` moving the listener onto
the network, and that is now visible from both ends. The server warns at startup, the
`server-info` frame carries an `exposed` flag, and the panel raises its `confirm` guard
once when it hears one — gating the destructive tools rather than the connection, which
is the shape this section already named as the least disruptive. Nothing about the
default local connect changed.

Closing it properly still means a handshake, and the cost to every local user is why it
has not been paid.
...
A **Language** preference was added later, declared in the manifest and refreshed on
`preferenceschange` — Figma does not re-render the Code panel by itself. A language with
nothing stored for the node falls back to showing every block, because a standing
preference must not read as "this node has no code".
