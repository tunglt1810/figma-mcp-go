...
// set_layout_sizing was the same nine sizing arguments over several nodes.
// set_auto_layout absorbed it by taking nodeIds, so putting one sizing on a row
// of siblings must still be a single call.
